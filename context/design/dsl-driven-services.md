...
The strategy for SQL in the reference architecture: the `sqlate` library, go-database's
infrastructure service and `admin` package over it, and go-web-service on authored SQL. It was
settled on 2026-08-29 across two sessions that reviewed go-database v0.3.0 (`ast`, `operation`,
`exec`) against the organization domain in go-web-service, and it was adjusted since; §10 lists
...
The library is its own repository and module, `github.com/standards-lab/sqlate`, first released
2026-09-04. Its base module imports only the standard library; a sourced dependency enters only
through a sub-module's `go.mod`. It owns everything from the `.sql` file to the scanned row:
the declaration grammar, the pattern catalog, compilation, the typed values that run a
statement, the struct-tag mapper, request directives over a header's declared fields, the
optimistic-concurrency guard, schema versioning in place of golang-migrate, the scripted
`database/sql` driver every consumer's unit tests run over, the conventions linter, and the
engine sub-modules that supply a dialect. Verification prepares every statement in a domain's
inventory against a live, migrated schema, so a renamed column fails at startup and in tests
rather than at the first request. The original plan kept `query` and `migrate` inside
go-database; the prototype's split rehearsal showed the library needs nothing of the service,
so a CLI, a worker, or a test harness can use authored SQL with no lifecycle machinery. The
library is adjacent to the standard rather than a layer of it: the standard's libraries consume
it, it is the blueprint for how a DSL gains host-language support, and its own text names
nothing of the architecture. The packages, the grammar, and the current version are the
repository's guide and CHANGELOG.

## 5. go-database

go-database keeps the infrastructure service (configuration with the pool settings and the
environment layer, the pool over the provider, lifecycle, readiness) and gained the `admin`
package: the database admin service that verifies, migrates, and seeds named states at startup
and on demand, every operation a trigger over a library function. Everything sqlate owns left
the module at v0.4.0; the `postgres` sub-module keeps the driver, the DSN, and pool
construction, and supplies no dialect. The admin service lives in go-database rather than
go-web-sdk because it depends on the pool, and a web SDK importing go-database would be an
upward dependency. Its HTTP half, the route group and handler, stays application code the
reference service builds: a documented pattern, never template scaffolding. The server-version
read is a dialect capability the `admin` package declares and `sqlate/postgres` implements, so
engine-native text stays with the engine. The packages, the wiring rule, and the lifecycle
stage are the module's `doc.go` comments; the versions are its CHANGELOG.

## 6. go-web-service

Built 2026-09-06 on the released libraries. The service's `context/design/domain-architecture.md`
carries the domain rules, and `internal/app/doc.go` the composition root.

### 6.1 A domain on authored SQL

Every statement is SQL in a file an editor can check, in a `statements/` directory per domain,
with a header that declares its tier. One file per domain imports the library: it compiles the
directory against the catalog, registers the inventory, binds each statement to its typed
value, and exposes each operation as a method that reads as what it does. A command's
validation belongs to the entity; existence and uniqueness belong to the store, as constraint
violations. A statement is named for its operation, never its SQL verb. The one advisory lock
statement is the service's, in its root-level `data` package beside the lock-name registry, so
the port list carries the mechanism once.

### 6.2 Startup

Schema versioning, verification, and seeding are library mechanisms the composition root
triggers, in three concerns kept separate because their risks differ. Verify is always on,
cheap, and safe: the migration history must be the embedded set's clean head, and every
statement must prepare against the live schema. Apply runs when verification finds pending
migrations, under the lock; a state the mechanism cannot correct fails startup, and an operator
resolves it through the admin endpoints. Seed applies a named set, the data a deployment or a
scenario starts from, declared by the service as data and applied idempotently; structural
reference data the schema itself needs is a migration. Whether a process that serves traffic
should ever hold DDL privileges is an open posture question (§9). The admin service owns the
first lifecycle stage; the domains verify their statements at the second.
...
Each `.sql` file declares its tier in its header. A standard file uses ISO/IEC 9075 forms
only. A native file names the engine feature it uses and how another engine expresses it, so
the native files in a repository are the complete work list for a port. The compiler reads the
header, the admin mount reports it, and `sqlint` checks it in CI: a standard file that matches
one of the engine's declared native forms fails the lint. This is the render-time capability
check of v0.3.0 translated into authored SQL, and the first phase of the meta-language
concept's compiler policy: a unit that uses a native feature declares it. The header grammar is
sqlate's guide.

## 7. The pattern catalog

A pattern is SQL that implements a shared rule, published as a `.sql` file with a tier and
placeholders under a namespace: the library's own under `sql`, an application's beside it, an
engine sub-module's overlay for the one pattern it spells differently. The composition root
builds the catalog once, every domain compiles its statements against it, and `sqlint.toml`
names the same sources so the linter resolves includes exactly as the runtime does. Admission
rule for a new pattern: at least one domain has needed it, its SQL shape is stated, and it
earns a Go function only if it carries a protocol the SQL cannot guarantee alone. The catalog
and its conventions are sqlate's guide.
