...
package's interface and constructor (`sqlate.Dialect` for SQL, with `database.New` for the pool), and any
...
- The roadmap decides where the port sits in the sequence and which engine it targets
  (`backlog.second-providers`); SQL Server is the engine the enterprise precedent points at.
- The `Dialect` interface, now `sqlate`'s (`design/dsl-driven-services.md`), grows by engine
  difference as the port reaches it; the port is what stresses the interface.
