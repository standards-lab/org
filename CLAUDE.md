...
here and kept coherent by the mirror convention above. The exception is scoped to the profiles;
...
context does not support. A claim in a profile must be true under the working context and the
architecture repository, in narrower words. Anything engagement-specific belongs in `private/`,
never in `org`. Check before committing.
