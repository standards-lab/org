...
// pass: twelve statements listed and eighteen prepares.
func TestStoreOverTheVariant(t *testing.T) {
	...
	stmts := s.Statements()
	if len(stmts) != 12 || stmts[10].Name() != "begin_file_delete" || stmts[11].Name() != "lock_tree" {
		var names []string
		for _, st := range stmts {
			names = append(names, st.Name()+":"+string(st.Tier()))
		}
		t.Errorf("Statements = %v, want the package's ten then the variant's two", names)
	}
	...
	if len(prepared) != 18 {
		t.Errorf("Verify prepared %d, want 18 (16 for the store, 2 for the variant)", len(prepared))
	}
	native := 0
