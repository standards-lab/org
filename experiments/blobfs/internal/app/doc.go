...
// the database and builds the logger and the output, admin.go the admin
// clients and their commands, and commands.go is the list of mounts. The
// domain layer's file arrives with the first domain command. Extending the
// application means editing a layer file's body; the signatures, cmd/blobfs,
// and App.Run stay untouched.
//
// New is the cold start, with no I/O: it builds the root command with its
// persistent flags, the infrastructure over the config, the admin layer
// over the infrastructure, and mounts each layer's commands on the root.
