...
type Domain struct {
	Volume func() (*volume.Store, error)
}
...
// commands are the tool's purpose, so volume, mkdir, and ls are root
// subcommands.
func mountDomain(dom *Domain, out *output.Output) []*cobra.Command {
	return volume.Commands(dom.Volume, out)
}
