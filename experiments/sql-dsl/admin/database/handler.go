...
// force, each a POST whose response is the resulting status. force sets the
// history without running any file — the operator's override for dirty
// state, after the schema has been repaired by hand. Every rejection is an
// RFC 9457 problem; a schema-state conflict carries its reason. The
...
// reject writes err as a problem. A schema-state conflict carries the
// error text as its detail: the SDK's writer sends text only on a 400, the
// right policy for a public API, but an operator surface needs to know
// which version is dirty or pending. Everything else follows the writer.
func (h *handler) reject(w http.ResponseWriter, r *http.Request, err error) {
	if code, ok := status(err); ok && code == http.StatusConflict {
...
// rejected request (400), a version outside the set (400), and the schema
// states an operation cannot proceed from — dirty, pending, a history the
// set does not carry, a migration with no down — as conflicts (409). A
// dialect without the lock capability stays unmatched: it is a wiring
// defect (500).
