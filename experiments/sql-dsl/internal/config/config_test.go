...
func TestSchemaConfig_DefaultsToVerify(t *testing.T) {
	cfg := configtest.Minimal()
	if err := cfg.Finalize(""); err != nil {
		t.Fatalf("Finalize: %v", err)
	}
	if cfg.Schema.Mode != config.SchemaVerify {
		t.Errorf("Schema.Mode = %q, want verify", cfg.Schema.Mode)
	}
	if cfg.Schema.Seeding() {
		t.Error("Seeding() = true by default, want false")
	}
}

func TestSchemaConfig_EnvOverridesAndValidation(t *testing.T) {
	t.Setenv("APP_SCHEMA_MODE", "apply")
	t.Setenv("APP_SCHEMA_SEED", "true")

	cfg := configtest.Minimal()
	if err := cfg.Finalize("app"); err != nil {
		t.Fatalf("Finalize: %v", err)
	}
	if cfg.Schema.Mode != config.SchemaApply || !cfg.Schema.Seeding() {
		t.Errorf("Schema = %+v, want apply with seeding", cfg.Schema)
	}

	t.Setenv("APP_SCHEMA_MODE", "sometimes")
	cfg = configtest.Minimal()
	err := cfg.Finalize("app")
	if err == nil || !strings.Contains(err.Error(), "schema:") {
		t.Errorf("error = %v, want the schema block wrap", err)
	}
}

func TestSchemaConfig_MergeOverlaysSetFields(t *testing.T) {
	on := true
	base := &config.Config{}
	base.Schema.Mode = config.SchemaVerify
	overlay := &config.Config{}
	overlay.Schema.Seed = &on

	base.Merge(overlay)

	if base.Schema.Mode != config.SchemaVerify {
		t.Errorf("Schema.Mode = %q, want the base value kept", base.Schema.Mode)
	}
	if !base.Schema.Seeding() {
		t.Error("Schema.Seed not overlaid")
	}
}
