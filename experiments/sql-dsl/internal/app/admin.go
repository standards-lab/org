...
// newAdmin wires the admin layer over infra. It takes lc because an admin
...
// statements.
func newAdmin(infra *Infrastructure, lc *lifecycle.Coordinator) (*Admin, error) {
	db, err := database.New(infra.SQL, infra.Logger)
	if err != nil {
		return nil, err
	}
	db.Register(lc)
