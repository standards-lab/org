...
func TestFiles_ParsesTheLayoutAndTheTransactionHeader(t *testing.T) {
	fsys := fstest.MapFS{
		...
		"m/0003_third.up.sql":   {Data: []byte("-- tier: native\n-- transaction: none\n\nCREATE INDEX CONCURRENTLY ix ON a (x)")},
	...
	if ms[2].Transactional || !strings.HasPrefix(ms[2].Up, "-- tier: native") {
		t.Errorf("third = %+v (header must opt out and the text must be kept whole)", ms[2])
	}
}
